...
void PclPassThroughEllipseFilter::filter(UpdatedHandles& /*input*/, const SensorPlugin* /*sensor*/) const
{
  if (cloud_pcl_handle_)
  {
    ...
    cloud_pcl_handle_->dispatch<l3::UniqueLock>([&](auto& cloud, auto type_trait) { cloud = filterPassThroughEllipse<decltype(type_trait)>(cloud, robot_pose, radius_x_, radius_y_); });
  }
}
}  // namespace l3_terrain_modeling
